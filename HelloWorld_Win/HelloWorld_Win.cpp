    ...
    _In_ int       nCmdShow) // 윈도우를 화면에 보여주는 방법을 명시
{
    ...
    LoadStringW(hInstance, IDC_WINDOWSPROJECT1, szWindowClass, MAX_LOADSTRING); // LoadString() API함수는 문자열 테이블에서 리소스 ID를 기준으로 문자열을 로딩하는 함수

    MyRegisterClass(hInstance); // 생성할 윈도우의 기본적인 정보를 담은 구조체를 완성하고 RegisterClassEx() API함수를 호출하여 윈도우 클래스를 등록
    ...
    // 엑셀러레이터 키 테이블의 핸들/ 응용프로그램 대부분은 바로 가기 키를 지원
    ...
    while (GetMessage(&msg, nullptr, 0, 0))
    {
        if (!TranslateAccelerator(msg.hwnd, hAccelTable, &msg))
        {
            ...
            DispatchMessage(&msg);// 내부적으로 메시지 프로시저 함수를 호출
		// 호출한 메시지 프로시저 함수는 전달받은 메시지가 무엇인지 분석하고 적절한 처리 수행
        }
    }
